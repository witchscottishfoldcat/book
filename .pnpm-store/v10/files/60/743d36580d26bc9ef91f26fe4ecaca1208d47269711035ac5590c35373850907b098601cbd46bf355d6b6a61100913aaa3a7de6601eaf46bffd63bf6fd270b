...
export interface UploadOptions {
    uploader: (files: FileList, schema: Schema, ctx: Ctx, insertPos: number) => Promise<Fragment | Node | Node[]>;
    enableHtmlFileUploader: boolean;
    uploadWidgetFactory: (pos: number, spec: Parameters<typeof Decoration.widget>[2]) => Decoration;
    getInsertPos?: (event: ClipboardEvent | DragEvent, ctx: Ctx, defaultInsertPos: number) => number;
}
